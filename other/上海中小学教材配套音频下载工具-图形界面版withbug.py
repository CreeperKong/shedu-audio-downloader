...
def download_file(url, base_dir, folder_format, code, title, silent, output_queue, lang):
    ...
    try:
        with requests.get(url, stream=True, timeout=20) as r:
            ...
            # --- FIX IS HERE: Re-added parameters to prevent GUI freezing/errors ---
            with open(file_path, 'wb') as f, tqdm(
                total=total_size, unit='iB', unit_scale=True, unit_divisor=1024,
                desc=desc_text, file=tqdm_out, disable=silent,
                dynamic_ncols=False, ascii=True,  # <-- FIX
...
class App:
    ...
    def show_code_menu(self, event):
        ...
        try:
            self.code_menu.entryconfig(d['menu_paste'], state=tk.NORMAL if self.root.clipboard_get() else tk.DISABLED)
        except tk.TclError:
            self.code_menu.entryconfig(d['menu_paste'], state=tk.DISABLED)

...
LANG = {
    'en': {
        ...
        'folder_format_label': "Sort into folders:", 'silent_check': "Silent (no progress bar)",
        ...
        'msg_request_error': "An error occurred for code {code}: {e}", 'tqdm_desc': "  -> {filename}",
        'folder_formats': {
    ...
    'zh': {
        ...
        'folder_format_label': "按照文件夹分类:", 'silent_check': "静默模式 (无进度条)",
        ...
        'msg_request_error': "处理提取码 {code} 时发生错误: {e}", 'tqdm_desc': "  -> {filename}",
        'folder_formats': {
...
class QueueIO:
    def __init__(self, q): self.q = q; self.buffer = ''
    def write(self, text):
        if '\r' in text: self.buffer = text.strip(); self.q.put(('progress', self.buffer))
        elif '\n' in text: self.q.put(('log', self.buffer)); self.buffer = ''
    def flush(self): pass
...
def download_file(url, base_dir, folder_format, code, title, silent, output_queue, lang):
    ...
    try:
        with requests.get(url, stream=True, timeout=20) as r:
            ...
            with open(file_path, 'wb') as f, tqdm(total=total_size, unit='iB', unit_scale=True, unit_divisor=1024, desc=desc_text, file=tqdm_out, disable=silent, bar_format='{desc}: {percentage:3.0f}%|{bar}| {n_fmt}/{total_n_fmt} [{elapsed}<{remaining}]') as bar:
                for chunk in r.iter_content(chunk_size=8192): bar.update(f.write(chunk))
    except Exception as e: output_queue.put(('error', LANG[lang]['msg_download_failed'].format(e=e)))
...
def fetch_and_parse_logic(code, should_download, options, output_queue):
    ...
    try:
        ...
        for link in shtml_links:
            ...
            if should_download: download_file(full_url, options['target_dir'], options['folder_format'], code, title, options['silent'], output_queue, lang)
    except Exception as e: output_queue.put(('error', LANG[lang]['msg_request_error'].format(code=code, e=e)))
...
class App:
    def __init__(self, root, initial_lang):
        ...
        self.log_text.tag_config('separator', foreground='lightcoral'); self.log_text.tag_config('progress', foreground='white')
        
        # --- NEW: Create and bind context menus ---
    ...
    def create_context_menus(self):
        # --- Menu for the code input box ---
        self.code_menu = tk.Menu(self.root, tearoff=0)
        # Commands are added in update_language to support translation
        
        # --- Menu for the log output box ---
        self.log_menu = tk.Menu(self.root, tearoff=0)

        # Bind right-click event to show the menus
    ...
    def show_code_menu(self, event):
        # Dynamically enable/disable items based on state
        has_selection = bool(self.codes_text.tag_ranges("sel"))
        self.code_menu.entryconfig(LANG[self.lang_var.get()]['menu_cut'], state=tk.NORMAL if has_selection else tk.DISABLED)
        self.code_menu.entryconfig(LANG[self.lang_var.get()]['menu_copy'], state=tk.NORMAL if has_selection else tk.DISABLED)
        self.code_menu.entryconfig(LANG[self.lang_var.get()]['menu_delete'], state=tk.NORMAL if has_selection else tk.DISABLED)
        try:
            self.code_menu.entryconfig(LANG[self.lang_var.get()]['menu_paste'], state=tk.NORMAL if self.root.clipboard_get() else tk.DISABLED)
        except tk.TclError:
            self.code_menu.entryconfig(LANG[self.lang_var.get()]['menu_paste'], state=tk.DISABLED)
    ...
    def update_language(self):
        ...
        # --- NEW: Rebuild menus with correct language and shortcuts ---
    ...
    def log_message(self, message, tag=None):
        ...
        if tag == self.progress_line_tag: self.log_text.delete(f"end-2l+1c", "end-1l")
    ...
    def process_queue(self):
        try:
            ...
            if msg_type == 'progress': self.log_message(msg_content, self.progress_line_tag)
            elif msg_type == 'finish': self.get_urls_button.config(state='normal'); self.download_button.config(state='normal')
            else: self.log_message(msg_content, msg_type)
